...
plot_sections <- function(merfish.anno, 
                          ...
                          ...)
  {
    ...
    rd.dat <- data.frame(
          x = merfish.anno[[x_coord]],
         y = merfish.anno[[y_coord]]
       )
    colnames(rd.dat) = c(x_coord, y_coord)
    ...
    # Global limits once
    xlims <- range(rd.dat[, x_coord], na.rm=TRUE)
    ylims <- range(rd.dat[, y_coord], na.rm=TRUE)
    ...
    meta.section = merfish.anno %>% 
      filter(cell_id %in% select.cells & section %in% sections) %>% 
      select(one_of(c(meta.f, "section"))) %>% 
      group_by_at(vars(one_of(meta.f)))  %>% 
      summarise(select.section = names(which.max(table(section))))
    ...
    for(sec in sections){
     # print(sec)
      ...
      # Center section within global box
      sec.cx <- mean(select.rd.dat[,x_coord], na.rm=TRUE)
      sec.cy <- mean(select.rd.dat[,y_coord], na.rm=TRUE)
      
      dx <- xmid - sec.cx
      dy <- ymid - sec.cy
      
      select.rd.dat[,x_coord] <- select.rd.dat[,x_coord] + dx
      select.rd.dat[,y_coord] <- select.rd.dat[,y_coord] + dy
