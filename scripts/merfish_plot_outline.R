                          ...
                          ...)
  {
    ...
    if(!is.null(plot.ccf)){
      for(sect in names(g.list)){
        ...
        plot.meta <- plot.meta %>% 
          left_join(hull.df[,c(x_coord,y_coord,"CCF_bin_x", "CCF_bin_y",ccf.column)], 
                    by=c("Dim1"=x_coord, "Dim2"=y_coord) ) 
        
        g.list[[sect]] <- g.list[[sect]] + #theme_bw() + 
          ggnewscale::new_scale_color() +
          geom_mark_hull(data = plot.meta %>% ungroup(), inherit.aes = F,
                         aes( x=Dim1, y=Dim2,
                              group = get(ccf.column), 
                              filter = get(ccf.column) %in% plot.ccf),
                         lwd = 0.1,
                         concavity = 5,
                         expand=0,
                         radius=0
...
sections = c("1199651066","1199651048", "1199651036","1199651018", "1199651005","1199650975","1199650968","1199650938")
